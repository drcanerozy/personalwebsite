---
title: "The factors associated with orthorexia nervosa in type 2 diabetes and their effect on diabetes self-management scores"
type: "publication"
category: "sci"
badge: "Eat Weight Disord (2023) • SCI-E"
authors_formatted: 'Kamarlı Altun, H., <strong class="text-slate-800">Özyıldırım, C.</strong>, Koç, Ş., Aksoy, H. N., Sağır, B., Bozkurt, M. S., & Karasu, H. (2023).'
journal_info: "Eating and Weight Disorders, 28, 1552."
year: 2023
date: "2023-01-06"
doi: "10.1007/s40519-023-01552-4"
url: "https://doi.org/10.1007/s40519-023-01552-4"
lay_summary: ""
bibtex: |
  @article{kamarli2023factors,
    title={The factors associated with orthorexia nervosa in type 2 diabetes and their effect on diabetes self-management scores},
    author={Kamarl{\i} Altun, H. and {\"O}zy{\i}ld{\i}r{\i}m, Caner and Ko{\c{c}}, {\c{S}}. and Aksoy, H. N. and Sa{\u{g}}{\i}r, B. and Bozkurt, M. S. and Karasu, H.},
    journal={Eating and Weight Disorders},
    volume={28},
    pages={1552},
    year={2023},
    doi={10.1007/s40519-023-01552-4}
  }
draft: false
lang: "tr"
order: 10
---
